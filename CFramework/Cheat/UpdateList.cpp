...
void CFramework::UpdateList()
{
    ...
    while (g.process_active)
    {
        ...
        if (g.g_ESP)
        {
            ...
            local.m_address = m.Read<uintptr_t>(tarkov.m_pLocalGameWorld + offset::MainPlayer);
            ...
            if (local.m_address != NULL)
            {
                ...
                const auto RegisteredPlayers = m.Read<uintptr_t>(tarkov.m_pLocalGameWorld + offset::RegisteredPlayers);
                ...
                auto pEntityList = m.Read<CGameObjectList>(EntityArray.list_address + 0x20);

                // ESP用EntityListを構築
                for (auto i = 0; i < EntityArray.count; i++)
                {
                    CPlayer p = CPlayer();
                    p.m_address = pEntityList.address[i];
...
void CFramework::UpdateStaticList() // C6262 :(
{
    while (g.process_active)
    {
        ...
        if (tarkov.Update())
        {
            ...
            if (g.g_ESP_Exfil)
            {
                const auto exfil_controller = m.Read<uintptr_t>(tarkov.m_pLocalGameWorld + offset::ExfilController);
