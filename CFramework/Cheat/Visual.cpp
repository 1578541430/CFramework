...
{
    return (vIn.x < g.GameRect.left || vIn.x > g.GameRect.right || vIn.y < g.GameRect.top || vIn.y > g.GameRect.bottom) ? false : true;
}

void CFramework::RenderInfo()
{
    // フレームカウンター
    std::string szFramerate{ std::to_string((int)ImGui::GetIO().Framerate) + "FPS" };
    g_gui->String(Vector2(3.f, 3.f), Col_Text_Default, szFramerate.c_str());

    // Crosshair
    if (g.g_Crosshair && !m_localplayer.IsAiming())
    {
        switch (g.g_CrosshairType)
        {
        case 0: {
            ImVec2 Center = ImVec2(g.GameRect.right / 2, g.GameRect.bottom / 2);
            ImGui::GetBackgroundDrawList()->AddLine(ImVec2(Center.x - g.g_CrosshairSize, Center.y), ImVec2((Center.x + g.g_CrosshairSize) + 1, Center.y), Col_Crosshair, 1);
            ImGui::GetBackgroundDrawList()->AddLine(ImVec2(Center.x, Center.y - g.g_CrosshairSize), ImVec2(Center.x, (Center.y + g.g_CrosshairSize) + 1), Col_Crosshair, 1);
        }   break;
        case 1:
            ImGui::GetBackgroundDrawList()->AddCircle(ImVec2((float)g.GameRect.right / 2.f, (float)g.GameRect.bottom / 2.f), g.g_CrosshairSize, Col_Crosshair, NULL);
            break;
        }
    }
}
...
{
    // Localの更新
    CPlayer* pLocal = &m_localplayer;
   
    if (!pLocal->Update())
        return;

    std::vector<CPlayer> entity_list;
    std::vector<CExfil> exfill_list;
    std::vector<CItem> item_list;

    {
        std::lock_guard<std::mutex> lock(m_mtx_fast);
        entity_list = m_vecEntityList;
    }


    {
        std::lock_guard<std::mutex> lock(m_mtx_slow);
        exfill_list = m_vecExfilList;
        item_list = m_vecItemList;
    }
    ...
    static Vector2 s_radar_pos{ g.GameRect.right - (s_radar_size.x + 25.f) , 25.f };
    ...
    */
    ...
    for (auto& entity : entity_list)
    {
        ...
        if (pEntity->m_iSpawnType == BTR_VEHICLE) {
            ...
            g_gui->String(Vector2(pScreen.x - (ImGui::CalcTextSize(BTR_Text.c_str()).x / 2.f), pScreen.y), ImColor(1.f, 1.f, 1.f, 1.f), BTR_Text.c_str());
        ...
        // Line
        if (g.g_ESP_Line)
            g_gui->Line(Vector2(g.GameRect.right / 2.f, g.GameRect.bottom), Vector2(pScreen.x, pScreen.y), pColor, 1.f);
        ...
        if (g.g_ESP_Box)
        {
            ...
            {
            case 0:
                g_gui->Rect(Vector2(pScreen.x - pWidth, pScreen.y - pHeight), Vector2(pScreen.x + pWidth, pScreen.y), pColor);
                break;
            case 1:
                g_gui->CorneredBox(Vector2(pScreen.x - pWidth, pScreen.y - pHeight), Vector2(pScreen.x + pWidth, pScreen.y), 3, pColor);
                break;
            default:
                break;
            }
        ...
        {
            // Head Circle
            g_gui->Circle(Vector2(pHead.x, pHead.y - (HeadToNeck / 2.f)), HeadToNeck * 1.55f, pColor);
            ...
            if (pEntity->m_vecBoneList.size() != 0)
            {
                ...
                for (int j = 0; j < 9; j++) {
                    ...
                    g_gui->Line(Vector2(bOut1.x, bOut1.y), Vector2(bOut2.x, bOut2.y), pColor, 1.f);
                }
            }
        }
        ...
        if (g.g_ESP_Distance) {
            ...
            g_gui->String(Vector2(pScreen.x - (ImGui::CalcTextSize(DistanceStr.c_str()).x / 2.f), pScreen.y), ImColor(1.f, 1.f, 1.f, 1.f), DistanceStr.c_str());
        }

        // Healthbar
        if (g.g_ESP_HealthBar) {
            pEntity->UpdateHealth();
            g_gui->HealthBar(pScreen.x - pWidth - 4.f, pScreen.y, 2.f, -pHeight, pEntity->m_fHealth, pEntity->m_fHealthMax);
        }
        ...
        if (g.g_ESP_Name && pEntity->m_iSpawnType != SIDE_SCAV && pEntity->m_iSpawnType != SNIPER_SCAV && pEntity->m_iSpawnType != NORMAL_SCAV)
        {
            ...
            g_gui->String(Vector2(pScreen.x - (ImGui::CalcTextSize(Name.c_str()).x / 2.f), pScreen.y - pHeight - 14.f), ImColor(1.f, 1.f, 1.f, 1.f), Name.c_str());
        }
    }
                ...
                for (auto k = 0; k < grenade_array.count; k++)
                {
                    ...
                    if (grenade != NULL)
                    {
                        ...
                        if (gDistance > 100.f)
                            ...
                            g_gui->String(Vector2(g.GameRect.right / 2.f - (ImGui::CalcTextSize("[ WARNING ] Grenade!!").x / 2.f), g.GameRect.bottom / 2.f), ImColor(1.f, 0.f, 0.f, 1.f), "[ WARNING ] Grenade!!");
                        ...
                        g_gui->String(Vector2(pGrenadeRoot.x - (ImGui::CalcTextSize(gre_tx.c_str()).x / 2.f), pGrenadeRoot.y - 13.f), ImColor(1.f, 0.f, 0.f, 1.f),gre_tx.c_str());
                    }
                }
            }
        }
    ...
    if (g.g_ESP_Item)
    {
        for (auto& item : item_list)
        {
            ...
            g_gui->String(Vector2(pItemScreen.x - (ImGui::CalcTextSize(vItemTx.c_str()).x / 2.f), pItemScreen.y), itemCol, vItemTx.c_str());
        }
    }
    ...
    if (g.g_ESP_Exfil)
    {
        for (auto& exfil : exfill_list)
        {
            ...
            g_gui->String(Vector2(pExfilScreen.x - (ImGui::CalcTextSize(exfilText.c_str()).x / 2.f), pExfilScreen.y), ExfilColor, exfilText.c_str());
        }
    }
}